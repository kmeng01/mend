...
class CFzsREDataset(Dataset):
    def __init__(self, split: str, tokenizer, config, edit_path: str,
                 pct: int = 10, max_length: int = 200):
        assert split in ["train", "validation"]

        with open("/home/mengk/mend/data/counterfact/zsre_cf.json", "r") as f:
            self.edit_samples = json.load(f)
            self.edit_samples = self.edit_samples[:-1000] if split == "train" else self.edit_samples[-1000:]
            # self.edit_samples = self.edit_samples[:1]
    ...
    def edit_generator(self, batch_size, n=None):
        ...
        while True:
            ...
            loc_target_toks = self.tok([" " + el["target"] for el in record["neighborhood_prompts"]])["input_ids"]

            edit_batch = [request["prompt"].format(request["subject"]) + self.tok.decode(target_new_tok[:-1])]
            loc_batch = [el["prompt"] + self.tok.decode(loc_target_toks[i][:-1]) for i, el in enumerate(record["neighborhood_prompts"])]
            ...
            loc = {**loc_toks}
            loc["labels"] = edit_labels(loc_toks, loc_target_toks[0])
